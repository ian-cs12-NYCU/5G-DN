...
echo "  允許從 UE 到 Web 容器的所有流量"
sudo iptables -A FORWARD -s $ANDY -d ${CIP} \
  -m conntrack --ctstate NEW,ESTABLISHED,RELATED -j ACCEPT

echo "  允許從 Web 容器回到 UE 的所有流量"
sudo iptables -A FORWARD -s ${CIP} -d $ANDY \
  -m conntrack --ctstate ESTABLISHED,RELATED -j ACCEPT

echo "  允許從 UE 到 MQTT 容器的所有流量"
sudo iptables -A FORWARD -s $ANDY -d ${MQTT_CIP} \
  -m conntrack --ctstate NEW,ESTABLISHED,RELATED -j ACCEPT

echo "  允許從 MQTT 容器回到 UE 的所有流量"
sudo iptables -A FORWARD -s ${MQTT_CIP} -d $ANDY \
  -m conntrack --ctstate ESTABLISHED,RELATED -j ACCEPT

echo -e "\n${YELLOW}設置 ICMP (ping) FORWARD 規則 (允許 ping) ...${NC}"

echo "  允許 ICMP (ping) 從 UE 到 Web 容器"
sudo iptables -A FORWARD -p icmp -s $ANDY -d ${CIP} -j ACCEPT

echo "  允許 ICMP (ping) 從 Web 容器到 UE"
sudo iptables -A FORWARD -p icmp -s ${CIP} -d $ANDY -j ACCEPT

echo "  允許 ICMP (ping) 從 UE 到 MQTT 容器"
sudo iptables -A FORWARD -p icmp -s $ANDY -d ${MQTT_CIP} -j ACCEPT
